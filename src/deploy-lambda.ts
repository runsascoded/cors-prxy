...
async function ensureRole(iam: IAMClient, roleName: string, tags: Record<string, string>): Promise<string> {
  try {
...
export async function deployLambda(config: CorsProxyConfig): Promise<DeployResult> {
  ...
  let endpoint: string
  try {
    const urlResp = await lambda.send(
