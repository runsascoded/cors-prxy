...
program
  ...
  .action(async (opts) => {
    ...
    const regions = opts.regions.split(",").map((r: string) => r.trim())
...
program
  ...
  .action(async (opts) => {
    ...
    if (proxy.region !== "global") console.log(`Region:   ${proxy.region}`)
    console.log(`Version:  ${proxy.version}`)
    if (proxy.repo) console.log(`Repo:     ${proxy.repo}`)
  })
...
program
  ...
  .action(async (opts) => {
    ...
    const printEvents = async (startTime?: number) => {
      try {
