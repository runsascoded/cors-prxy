...
describe("full proxy mode", () => {
    ...
    it("parses path-mode URLs", async () => {
        ...
        vi.stubGlobal("fetch", mockFetch);
        await handleProxyRequest({ method: "GET", url: "/example.com/v1/users?limit=10", origin: "https://app.example.com" }, fullProxyConfig);
        expect(mockFetch).toHaveBeenCalledWith("https://example.com/v1/users?limit=10", expect.anything());
    });
    it("returns 400 for invalid path-mode URL", async () => {
        const res = await handleProxyRequest({ method: "GET", url: "/", origin: "https://app.example.com" }, fullProxyConfig);
        expect(res.status).toBe(400);
        expect(JSON.parse(res.body).error).toBe("Invalid path — expected /<host>/<path>");
    });
    ...
    it("advertises allowed methods and headers in CORS preflight", async () => {
        const res = await handleProxyRequest({ method: "OPTIONS", url: "/", origin: "https://app.example.com" }, fullProxyConfig);
        expect(res.status).toBe(204);
        expect(res.headers["access-control-allow-methods"]).toBe("GET, HEAD, POST, PUT, DELETE, PATCH, OPTIONS");
    ...
    it("allows selective methods", async () => {
        ...
        const res = await handleProxyRequest({ method: "DELETE", url: "/?url=https://example.com/x", origin: "https://app.example.com" }, selectiveConfig);
        expect(res.status).toBe(405);
        vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response("ok", {
            status: 200,
            headers: { "content-type": "text/plain" },
        })));
        const res2 = await handleProxyRequest({ method: "POST", url: "/?url=https://example.com/x", origin: "https://app.example.com", body: "data" }, selectiveConfig);
