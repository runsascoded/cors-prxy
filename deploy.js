...
export async function findDestroyTargets(name, region, runtimeFilter) {
    const targets = [];
    if (!runtimeFilter || runtimeFilter === "lambda") {
        try {
            ...
            if (hasFunction || hasIamRole) {
                const parts = [];
                if (hasFunction)
                    parts.push(`Lambda: ${name} (${region})${urlInfo}`);
                if (hasIamRole)
                    parts.push(`IAM role: ${roleName}`);
                targets.push({
                    runtime: "lambda",
                    name,
                    detail: parts.join(" + "),
...
export async function destroyByRuntime(runtime, name, config) {
    if (runtime === "cloudflare") {
...
export async function destroy(config, runtimeFilter) {
