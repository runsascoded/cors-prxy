...
function getCfAuth() {
    ...
    if (!apiToken) {
        throw new Error("CLOUDFLARE_API_TOKEN env var is required for Cloudflare deploys.\n" +
...
async function cfApi(path, apiToken, opts = {}) {
...
export async function deployCf(config) {
    ...
    const existing = await cfApi(`/accounts/${accountId}/workers/scripts/${workerName}`, apiToken);
    ...
    const uploadResp = await cfApi(`/accounts/${accountId}/workers/scripts/${workerName}`, apiToken, {
        method: "PUT",
    ...
    await cfApi(`/accounts/${accountId}/workers/scripts/${workerName}/subdomain`, apiToken, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ enabled: true }),
    });
    // Get the subdomain
    const subdomainResp = await cfApi(`/accounts/${accountId}/workers/subdomain`, apiToken);
    const subdomain = subdomainResp.result?.subdomain ?? accountId;
    ...
    if (config.cloudflare?.route) {
        ...
        await cfApi(`/accounts/${accountId}/workers/scripts/${workerName}/routes`, apiToken, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
...
export async function listCfWorkers(accountId, apiToken) {
    const resp = await cfApi(`/accounts/${accountId}/workers/scripts`, apiToken);
    if (!resp.success)
        return [];
    const scripts = resp.result;
...
//# sourceMappingURL=deploy-cf.js.map